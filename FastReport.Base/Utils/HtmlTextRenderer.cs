...
namespace FastReport.Utils
{
    public class HtmlTextRenderer : IDisposable
    {
        ...
        private string text;
        private SkiaSharp.SKColor underlineColor;
        ...
        public SkiaSharp.SKRect DisplayRect { get { return displayRect; } }
        ...
        {
            get { 
                return !(format.FormatFlags == StringFormatFlags.NoWrap); 
                }
        }
       ...
       /// <param name="context"></param>
        public HtmlTextRenderer(RendererContext context)
        {
            ...
            this.format.HotkeyPrefix = HotkeyPrefix.None;
        ...
        public HtmlTextRenderer(string text, SkiaSharp.SKCanvas g, SkiaSharp.SKTypeface font, float size,
                    ...
                    ParagraphFormat paragraphFormat, bool forceJustify, float scale, float fontScale, InlineImageCache cache, bool isPrinting = false, bool isDifferentTabPositions = false)
        {
            ...
            // Dispose it
            this.format = new StringFormat();//TODOStringFormat.GenericTypographic.Clone() as StringFormat;
            if (RightToLeft)
                this.format.FormatFlags |= StringFormatFlags.DirectionRightToLeft;
            float firstTab;
...
#region Internal Methods
        ...
        internal void Draw()
        {
            ...
            {
                using (var brush = new SkiaSharp.SKPaint())
                {
                    brush.Color = rect.Color;
                    graphics.DrawRect(rect.Left, rect.Top, rect.Width, rect.Height,brush);
                }
            }
            ...
            {
                using  (var pen = new /*Pen*/SkiaSharp.SKPaint ())
                {
                    pen.Color = line.Color;
                    pen.StrokeWidth = line.Width;
                    graphics.DrawLine(line.Left, line.Top, line.Right, line.Top, pen);
                }
            }
...
#region Private Methods
        ...
        private void CssStyle(StyleDescriptor style, Dictionary<string, string> dict)
        {
            ...
            if (dict.TryGetValue("color", out tStr))
            {
                ...
                else style.Color = (SkiaSharp.SKColor)typeof(SkiaSharp.SKColors).GetFields(System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.Public).Where(x=> x.Name == tStr).FirstOrDefault().GetValue(null);
            }
            ...
            if (dict.TryGetValue("background-color", out tStr))
            {
                ...
                else style.BackgroundColor = (SkiaSharp.SKColor)typeof(SkiaSharp.SKColors).GetFields(System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.Public).Where(x=> x.Name == tStr).FirstOrDefault().GetValue(null);
            }
        }
                ...
                else
                {
                    ...
                    if (!element.IsSelfClosed)
                    {
                        ...
                        for (int i = arr.Length - 1; i >= 0; i--)
                        {
                            ...
                            {
                                case "b":
                                    newStyle.FontStyle = newStyle.FontStyle == SkiaSharp.SKFontStyle.Italic ? SkiaSharp.SKFontStyle.BoldItalic : SkiaSharp.SKFontStyle.Bold;
                                    break;

                                case "i":
                                    newStyle.FontStyle = newStyle.FontStyle == SkiaSharp.SKFontStyle.Bold ? SkiaSharp.SKFontStyle.BoldItalic : SkiaSharp.SKFontStyle.Italic;
                                    break;
...
#region Public Fields
            ...
            public readonly SkiaSharp.SKColor Color;
...
#region Public Constructors
            ...
            public LineFColor(float left, float top, float right, float width, SkiaSharp.SKColor color)
            {
                this.Left = left;
                this.Top = top;
                this.Right = right;
...
#region Public Constructors
            ...
            public RectangleFColor(float left, float top, float width, float height, SkiaSharp.SKColor color)
            {
                this.Left = left;
                this.Top = top;
                this.Width = width;
...
#region Public Constructors
            ...
            //    {
            //        using (Brush /*Brush*/SkiaSharp.SKPaint = GetBackgroundBrush())
...
#region Protected Methods
            ...
            //{
            //    if ((FStyle.FontStyle & SkiaSharp.SKFontStyle.Underline) == SkiaSharp.SKFontStyle.Underline)
            //    {
            //        if (!FRenderer.FUnderLines)
            //        {
            //            float top = Top + FUnderline;
            //            using  (/*Pen*/SkiaSharp.SKPaint pen = new /*Pen*/SkiaSharp.SKPaint (FStyle.Color, FUnderlineSize * 0.1f))
            ...
            //        }
            //    }
            //    if ((FStyle.FontStyle & SkiaSharp.SKFontStyle.Strikeout) == SkiaSharp.SKFontStyle.Strikeout)
            //    {
            //        float top = Top + FBaseLine / 3 * 2;
            //        using  (/*Pen*/SkiaSharp.SKPaint pen = new /*Pen*/SkiaSharp.SKPaint (FStyle.Color, FStyle.Size * 0.1f))
            //            if (FRenderer.FRightToLeft)
            //                FRenderer.FGraphics.DrawLine(pen, Left - Width, top, Left, top);
            //            else
            //                FRenderer.FGraphics.DrawLine(pen, Left, top, Left + Width, top);
...
#region Public Constructors
            ...
            public RunImage(HtmlTextRenderer renderer, Word word, string src, StyleDescriptor style, float left, int charIndex, float img_width, float img_height) : base(renderer, word, style, left, charIndex)
            {
                ...
                //TODO base.style.FontStyle &= ~(FontStyle.Strikeout | SkiaSharp.SKFontStyle.Underline);
                ...
                {
                    var p = new SkiaSharp.SKPaint(ff);
                    float height = p.TextSize;
                    float lineSpace = p.FontSpacing;
                    float descent = ff.Metrics.Descent;
...
#region Internal Methods
            ...
            public SkiaSharp.SKBitmap GetBitmap(out float width, out float height)
            {
                width = 1;
                height = 1;
                if (image == null)
            ...
            //                try
            //                {
            //                    using (SkiaSharp.SKBitmap bmp = new Bitmap(FImage.Width, FImage.Height))
            //                    {
            ...
            public RunText(HtmlTextRenderer renderer, Word word, StyleDescriptor style, List<CharWithIndex> text, float left, int charIndex) : base(renderer, word, style, left, charIndex)
            {
                ...
                using (var ff = style.GetFont())
                {
                    ...
                    if (this.text.Length > 0)
                    {
                        ...
                        if (word.Type == WordType.WhiteSpace)
                        {
                            //using (Font f = new SkiaSharp.SKFont("Consolas", 10))
                            width = CalcSpaceWidth(this.text, ff);
                        }
                        else
                        {
                            
                            width = p.MeasureText(this.text);
...
#region Public Methods
            ...
            //public override void Draw(bool drawContents)
            {
                using (SkiaSharp.SKFont font = style.GetFont())
                using (var brush = GetBrush())
                {
                    //if (drawContents)
                    //{
                    //#if DEBUG
                    //                    SkiaSharp.SKSize size = FRenderer.FGraphics.MeasureString(FText, font, int.MaxValue, FRenderer.FFormat);
                    ...
                    //#endif
                    renderer.graphics.DrawText(text, Left, Top, brush);
                }
                //}
                //base.Draw(drawContents);
            }
...
#region Public Properties
            ...
            public SkiaSharp.SKColor BackgroundColor
            {
            ...
            public SkiaSharp.SKTypeface Font
            {
                get { return font; }
                set { font = value; }
            }
...
#region Public Constructors
            ...
            public StyleDescriptor(SkiaSharp.SKFontStyle fontStyle, SkiaSharp.SKColor color, BaseLine baseLine, SkiaSharp.SKTypeface font, float size)
            {
...
#region Public Methods
            ...
            public SkiaSharp.SKFont GetFont()
            {
                ...
                return new SkiaSharp.SKFont(SkiaSharp.SKTypeface.FromFamilyName(font.FamilyName, fontStyle), fontSize);
            }
            ...
            public void ToHtml(FastString sb, bool close)
            {
                ...
                if (close)
                {
                    ...
                    if (Strikeout) sb.Append("</strike>");
                    if (Underline) sb.Append("</u>");
                    if (fontStyle == SkiaSharp.SKFontStyle.Italic|| fontStyle == SkiaSharp.SKFontStyle.BoldItalic) sb.Append("</i>"); 
                    if (fontStyle == SkiaSharp.SKFontStyle.Bold || fontStyle == SkiaSharp.SKFontStyle.BoldItalic) sb.Append("</b>");
